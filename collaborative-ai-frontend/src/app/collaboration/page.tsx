...
function CollaborationContent() {
    ...
    const [activeChannelPref, setActiveChannelPref] = useState('mentions');

    useEffect(() => {
        if (activeChatId && user) {
            const key = `channel_notif_pref_${user.id}_${activeChatId}`;
            setActiveChannelPref(localStorage.getItem(key) || 'mentions');
        }
    }, [activeChatId, user]);
    ...
    const fetchPinnedMessages = async (chatId?: string) => {
        ...
        try {
            const data = await apiClient.get(`/collaboration/conversations/${id}/pinned`);
            if (data) {
    ...
    const handleRemoveMember = async (targetUserId: string) => {
        if (!activeChatId) return;
        if (!window.confirm('Are you sure you want to remove this user from the channel?')) {
            return;
        }
        try {
            await apiClient.delete(`/collaboration/channels/${activeChatId}/members`, { userId: targetUserId });
    ...
    const handleSaveNotificationPreference = (pref: string) => {
        if (!activeChatId || !user) return;
        const key = `channel_notif_pref_${user.id}_${activeChatId}`;
        localStorage.setItem(key, pref);
        setActiveChannelPref(pref);
    ...
    useEffect(() => {
        ...
        if (typeof window !== 'undefined') {
            const isProd = window.location.hostname.includes('vercel.app') || window.location.hostname.includes('collaborative-ai');
            if (isProd) {
                wsUrl = `wss://collaborative-ai-platform.vercel.app/ws?token=${token}`;
            } else {
                wsUrl = `ws://${window.location.hostname}:5001?token=${token}`;
            }
        }
            ...
            ws.onmessage = (event) => {
                try {
                    ...
                    if (data.type === 'message') {
                        ...
                        if (newMsg.conversationId === activeChatIdRef.current) {
                            setMessages(prev => {
                                if (prev.some(m => m.id === newMsg.id)) return prev;
                                return [...prev, newMsg];
                            });
                            apiClient.post(`/collaboration/conversations/${activeChatIdRef.current}/read`, {}).catch(() => { });
                        } else {
                            setConversations(prev => {
                                const matchedConv = prev.find(c => c.id === newMsg.conversationId);
                                const prefKey = `channel_notif_pref_${userRef.current?.id}_${newMsg.conversationId}`;
                                const pref = localStorage.getItem(prefKey) || 'mentions';
                                const isDM = matchedConv?.type === 'direct';
                                
                                if (pref === 'all' || (isDM && pref !== 'mute')) {
                                    const convName = matchedConv?.type === 'group' ? `#${matchedConv.name}` : newMsg.sender.name;
                                    showToast(`New message in ${convName}: ${newMsg.content.substring(0, 40)}${newMsg.content.length > 40 ? '...' : ''}`, 'info');
                                }
                    ...
                    if (data.type === 'read') {
                        ...
                        if (conversationId === activeChatIdRef.current) {
                            setMessages(prev => prev.map(msg => {
                                if (msg.senderId === userRef.current?.id && new Date(msg.createdAt) <= new Date(readAt)) {
                    ...
                    if (data.type === 'typing') {
                        ...
                        if (conversationId === activeChatIdRef.current && senderId !== userRef.current?.id) {
                            if (isTyping) {
                                const typingUser = directoryUsersRef.current.find(u => u.id === senderId)?.name || 'Someone';
    ...
    return (
        <div className="dm-container animate-fade-in">
                ...
                <div className="dm-left-panel-new">
                    <div className="dm-left-header-new">
                        <button
                            ...
                            onClick={() => role === 'Admin' && setIsChannelModalOpen(true)}
                            style={{ cursor: role === 'Admin' ? 'pointer' : 'default' }}
                        >
                            {role === 'Admin' && <Plus size={16} />}
                        ...
                        <>
                            ...
                            <div className="dm-center-header-new">
                                ...
                                <div className="dm-header-actions-new">
                                    ...
                                    {activeConversation.type === 'group' && (
                                        <button className="icon-btn-text" onClick={() => { loadDirectoryUsers(); setIsAddMemberModalOpen(true); }} title="View Members" style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-secondary)', padding: '0 0.5rem' }}>
                                            <Users size={18} />
                                    ...
                                    {isHeaderMenuOpen && (
                                        <div className="dm-dropdown-menu-new">
                                            ...
                                            {activeConversation.type === 'group' && role === 'Admin' && (
                ...
                {activeChatId && activeConversation && isProfileOpen && (
                    <div className="dm-right-panel-new">
                        <div className="dm-right-panel-header-new">
                            <h3>{activeConversation.type === 'group' ? 'Channel Details' : 'User Details'}</h3>
                        ...
                        <div className="dm-right-scrollable-content">
                            ...
                            <div className="dm-right-card-header-new">
                                <div className="dm-right-big-avatar-new">
                                    {activeConversation.type === 'group' ? (
                                        ...
                                        <span style={{ fontWeight: 600, fontSize: '1.25rem' }}>{getInitials(activeConversation.partner?.name || '')}</span>
                                    )}
                                </div>
                                <h4>
                                    {activeConversation.type === 'group' 
                                ...
                                <p className="dm-right-card-description">
                                    {activeConversation.type === 'group' 
                                        ? (activeConversation.description || 'Discuss business updates, strategy, and goals') 
                                        : `${activeConversation.partner?.role || 'Member'} • ${activeConversation.partner?.email}`}
                                </p>
                                <p className="dm-right-card-created-by">
                                    {activeConversation.type === 'group' 
                                        ? 'Created by Admin on May 10, 2024' 
                                        : `Status: ${activeConversation.partner?.online ? 'Online' : 'Offline'}`}
                                </p>
                            </div>
                            ...
                            <div className="dm-right-actions-group-new">
                                {activeConversation.type === 'group' && (
                                    <button className="dm-right-action-tile-btn" onClick={() => { loadDirectoryUsers(); setIsAddMemberModalOpen(true); }}>
                                        <Plus size={18} />
                                ...
                                <button className="dm-right-action-tile-btn" onClick={() => setIsNotificationSettingsOpen(true)}>
                                    <Bell size={18} />
                            ...
                            <div className="dm-right-section-new">
                                <div className="dm-right-section-title-row">
                                    ...
                                    {activeConversation.type === 'group' && (
                                        <button className="dm-right-view-all-link" onClick={() => { loadDirectoryUsers(); setIsAddMemberModalOpen(true); }}>View all</button>
                                    )}
                                </div>
                                <div className="dm-right-members-list-new">
                                    ...
                                    {activeConversation.type === 'group' && activeConversation.participants && activeConversation.participants.length > 4 && (
                                        <button className="dm-right-more-members-btn" onClick={() => { loadDirectoryUsers(); setIsAddMemberModalOpen(true); }}>
                                ...
                                >
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flex: 1 }}>
                                        ...
                                        <div className="dm-modal-user-info" style={{ flex: 1 }}>
                                            <span className="dm-modal-user-name" style={{ fontWeight: 600 }}>{u.name}</span>
                                            <span className="dm-modal-user-role" style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>{u.role}</span>
                                        </div>
                                    </div>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                        {isAlreadyIn ? (
                                            <>
                                                <span className="dm-right-member-badge member-badge" style={{ backgroundColor: 'var(--bg-secondary)', color: 'var(--text-secondary)' }}>Joined</span>
                                                {role === 'Admin' && (
                                                    <button 
                                                        ...
                                                        onMouseOver={(e) => { e.currentTarget.style.backgroundColor = 'var(--danger-color)'; e.currentTarget.style.color = 'white'; }}
                                                        onMouseOut={(e) => { e.currentTarget.style.backgroundColor = 'rgba(239, 68, 68, 0.08)'; e.currentTarget.style.color = 'var(--danger-color)'; }}
                                                    >
                                                        Remove
                                                    </button>
                                                )}
                                            ...
                                            <button 
                                                ...
                                                onMouseOut={(e) => { e.currentTarget.style.backgroundColor = 'var(--primary-light)'; e.currentTarget.style.color = 'var(--primary-color)'; }}
                                            >
                                                Add
                                            </button>
                                        )}
            ...
            <Modal
                ...
                title={`Notification Settings: #${activeConversation?.name || 'Channel'}`}
            >
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginTop: '0.5rem' }}>
                    <p style={{ fontSize: '0.875rem', color: 'var(--text-secondary)' }}>
                        Choose how you want to be notified of new activity in this channel.
                    </p>
                    
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                        {[
                            { value: 'all', label: 'All Messages', desc: 'Get notified for every message sent in this channel' },
                            { value: 'mentions', label: 'Mentions Only', desc: 'Get notified only when you are @mentioned' },
                            { value: 'mute', label: 'Muted', desc: 'Disable all notifications for this channel' }
                        ].map(opt => {
                            const isSelected = activeChannelPref === opt.value;
                            return (
                                <div 
                                    key={opt.value}
                                    onClick={() => handleSaveNotificationPreference(opt.value)}
                                    style={{
                                        display: 'flex',
                                ...
                                >
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                        <span style={{ fontWeight: 600, color: 'var(--text-primary)', fontSize: '0.9rem' }}>{opt.label}</span>
                                        {isSelected && <Check size={16} style={{ color: 'var(--primary-color)' }} />}
                                    </div>
                                    <span style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginTop: '2px' }}>{opt.desc}</span>
                                </div>
                            );
                        })}
                    </div>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '0.5rem' }}>
                        <Button onClick={() => setIsNotificationSettingsOpen(false)}>Close</Button>
                    </div>
                </div>
            </Modal>
        </div>
