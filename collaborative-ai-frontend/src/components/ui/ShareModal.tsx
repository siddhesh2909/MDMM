...
import { 
    Users, UserPlus, Shield, Globe, Lock, Trash2, Loader2, Search, Check, ChevronDown
} from 'lucide-react';
...
export function ShareModal({ isOpen, onClose, datasetId, datasetName, onSaveCallback }: ShareModalProps) {
    ...
    const fetchSharingDetails = async () => {
        setLoading(true);
        try {
            const data = await apiClient.get(`/data/datasets/${datasetId}/share/users`);
            if (data) {
            ...
            showToast(err.message || 'Failed to load sharing permissions.', 'error');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleVisibilityChange = async (newVisibility: string) => {
        ...
        try {
            await apiClient.post(`/data/datasets/${datasetId}/share`, {
                visibility: newVisibility
            });
            setVisibility(newVisibility);
            showToast(`Dataset visibility updated to ${newVisibility}.`, 'success');
    ...
    const handleAddCollaborator = async () => {
        ...
        try {
            ...
            showToast(`Shared dataset with ${selectedUser.name}.`, 'success');
    ...
    const handleUpdatePermission = async (userId: string, permission: string) => {
        setActionLoading(true);
        try {
            await apiClient.post(`/data/datasets/${datasetId}/share/update`, {
                targetUserId: userId,
                permission
            });
    ...
    const handleRevokeShare = async (userId: string) => {
        ...
        setActionLoading(true);
        try {
            await apiClient.post(`/data/datasets/${datasetId}/share/revoke`, {
                targetUserId: userId
            });
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={`Share "${datasetName}"`} maxWidth="560px">
