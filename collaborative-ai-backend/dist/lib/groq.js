...
function handleMockCompletion(params) {
    ...
    if (isBusiness) {
        ...
        const isGreeting = words.some((w) => ['hi', 'hello', 'hey', 'greetings', 'welcome'].includes(w));
        if (isGreeting) {
            const revKpi = businessKpis.find((k) => k.metricName.toLowerCase().includes('revenue')) || businessKpis[0];
            const activeBuyersKpi = businessKpis.find((k) => k.metricName.toLowerCase().includes('buyer') || k.metricName.toLowerCase().includes('user')) || businessKpis[2];
            const qualityKpi = businessKpis.find((k) => k.metricName.toLowerCase().includes('quality')) || businessKpis[3];
...
How can I assist you with business KPI exploration, performance anomalies, or regional revenue analysis today?`;
            ...
            const revKpi = businessKpis.find((k) => k.metricName.toLowerCase().includes('revenue')) || businessKpis[0];
            const negativeKpis = businessKpis.filter((k) => k.delta.startsWith('-') || k.isGrowth === false);
            let negativeMetricsStr = '';
            if (negativeKpis.length > 0) {
                negativeMetricsStr = negativeKpis.map((k) => `• **${k.metricName}** has dropped by **${k.delta}** (current value: **${k.val}**).`).join('\n');
            }
            else {
...
What business KPIs would you like to explore next?`;
            ...
            const sortedRegions = [...businessRegions].sort((a, b) => {
...
${businessProducts.map((p, i) => `• **${i + 1}. ${p.name}**: Sales of **${p.sales}** (**${p.share}** market share)`).join('\n')}
...
What business KPIs would you like to explore next?`;
            ...
            const revKpi = businessKpis.find((k) => k.metricName.toLowerCase().includes('revenue')) || businessKpis[0];
            const buyersKpi = businessKpis.find((k) => k.metricName.toLowerCase().includes('buyer') || k.metricName.toLowerCase().includes('user')) || businessKpis[2];
...
• **Columns Present:** ${dsCols.map((c) => `**${c}**`).join(', ')}.
...
Use the capability filters on the left or type any technical query about column distributions, anomalies, or standardizations!`;
            ...
            const hasSales = dsCols.some((c) => ['spent', 'price', 'sales', 'revenue', 'amount', 'cost', 'rep', 'channel'].some(k => c.toLowerCase().includes(k)));
            const hasUser = dsCols.some((c) => ['user', 'customer', 'member', 'email', 'age', 'gender', 'signup'].some(k => c.toLowerCase().includes(k)));
            const hasProduct = dsCols.some((c) => ['product', 'item', 'category', 'sku', 'inventory'].some(k => c.toLowerCase().includes(k)));
...
• **Attributes List:** ${dsCols.slice(0, 5).map((c) => `**${c}**`).join(', ')}, and ${dsCols.length > 5 ? `${dsCols.length - 5} others` : 'no others'}.
...
${dsCols.map((col) => {
                ...
                if (lowerCol === 'id' || lowerCol === 'user_id' || lowerCol.includes('key'))
                    type = 'String (UUID)';
                else if (lowerCol.includes('age') || lowerCol.includes('spent') || lowerCol.includes('amount') || lowerCol.includes('price') || lowerCol.includes('quantity'))
                    type = 'Numeric (Float/Int)';
                else if (lowerCol.includes('date') || lowerCol.includes('time'))
                    type = 'Temporal (Date)';
                else if (lowerCol.includes('is_') || lowerCol.includes('active') || lowerCol.includes('status'))
                    type = 'Boolean';
