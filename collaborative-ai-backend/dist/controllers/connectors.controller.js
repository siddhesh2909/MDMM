...
exports.pullData = exports.testConnection = void 0;
...
const pullData = async (req, res) => {
    try {
        ...
        const mode = (['strict', 'warning', 'monitor'].includes(req.body.enforcementMode)
...
function generatePostgresData(table) {
    const now = new Date();
    const tables = {
...
function generateMongoData(collection) {
    const now = new Date();
    const collections = {
...
function generateMySQLData(table) {
    const now = new Date();
    const tables = {
